...
// Daily Output Routes
app.get('/page/manufacture/daily_output', async (req, res) => {
  try {
    const dbData = {
    ...
    // Call the function to handle new daily output
    await dailyOutputNew(req, res, dbData);
  } catch (error) {
    console.error('Error loading daily output page:', error);
    res.status(500).send('Server error');
  }
});

app.get('/page/manufacture/daily_output/view/:id', async (req, res) => {
  try {
    const dbData = {
    ...
    // Call the function to handle viewing daily output
    await dailyOutputView(req, res, dbData);
  } catch (error) {
    console.error('Error loading daily output view:', error);
    res.status(500).send('Server error');
  }
});

app.get('/page/manufacture/daily_output/edit/:id', async (req, res) => {
  try {
    const dbData = {
      txn_id: req.params.id,
      txn_mode: 'daily_edit',
    ...
    // Call the function to handle editing daily output
    await dailyOutputEdit(req, res, dbData);
  } catch (error) {
    console.error('Error loading daily output edit:', error);
    res.status(500).send('Server error');
  }
});
...
// Route for daily output inquiry page
